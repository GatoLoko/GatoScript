        ...
        if word[2] in protegidos:
            if disparador.search(word[3][1:]):
                ...
                if len(partes) == 1:
                    ...
                    if len(lista) == 0:
                        xchat.command("say Aun no hay torrents disponibles")
                    elif len(lista) > 0:
                        archivos = ""
                        for archivo in lista:
                            archivos += archivo + " "
                        xchat.command("say Tengo los siguientes torrents: " + archivos)
                        xchat.command("say Para pedir uno pon: !torrent nombre")
                elif len(partes) == 2:
                    lista = listdir(almacen)
                    ...
                    xchat.command("say ¡No seas abuson! ¡Un solo archivo!")
